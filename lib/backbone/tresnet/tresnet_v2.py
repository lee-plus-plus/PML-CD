...
def InplacABN_to_ABN(module: nn.Module) -> nn.Module:
    ...
    if isinstance(module, InPlaceABN):
        module_new = ABN(module.num_features, activation=module.activation, activation_param=module.activation_param)
...
def conv2d(ni, nf, stride):
    return nn.Sequential(nn.Conv2d(ni, nf, kernel_size=3, stride=stride, padding=1, bias=False), nn.BatchNorm2d(nf),
        ...
        if stride == 1:
            ...
            if anti_alias_layer is None:
                ...
                self.conv2 = nn.Sequential(
                    conv2d_ABN(planes, planes, kernel_size=3, stride=1, activation="leaky_relu",
                               activation_param=1e-3), anti_alias_layer(channels=planes, filt_size=3, stride=2))
...
class TResNetV2(Module):
    ...
    def __init__(self, layers, in_chans=3, num_classes=1000, width_factor=1.0, remove_model_jit=False):
        ...
        self.body = nn.Sequential(
            OrderedDict([('SpaceToDepth', SpaceToDepth), ('conv1', conv1), ('layer1', layer1), ('layer2', layer2),
    ...
    def _make_layer(self, block, planes, blocks, stride=1, use_se=True, anti_alias_layer=None):
        ...
        layers = []
        layers.append(
            block(self.inplanes, planes, stride, downsample, use_se=use_se, anti_alias_layer=anti_alias_layer))
...
def TResnetL_V2(num_classes=196, remove_model_jit=False):
    """Constructs a large TResnet model.
    """
    in_chans = 3
    layers_list = [3, 4, 23, 3]
    width_factor = 1.0
    model = TResNetV2(layers=layers_list, num_classes=num_classes, in_chans=in_chans, width_factor=width_factor,
